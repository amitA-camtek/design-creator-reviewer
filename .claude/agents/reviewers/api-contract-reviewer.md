...
2. Read `api-design.md` front-matter: `api_binding`, `api_auth`, `sensitive_fields`, `required_endpoints`.
3. Read `architecture-design.md` front-matter: `service_name`, `primary_language`.
4. Read `schema-design.md` front-matter: `storage_technology`.
5. If design files are not found, proceed with best-effort review; note the gap.

## Your responsibilities

### 1. Binding and port
- Verify the API server is configured to bind exactly to the address and port specified in `api_binding` from api-design.md front-matter.
...
- Verify all endpoints listed in `required_endpoints` from api-design.md front-matter exist with the correct HTTP method and path.
...
- If the api-design.md front-matter describes the API as read-only (no write endpoints in `required_endpoints`), verify that no write operations (INSERT/UPDATE/DELETE/DDL) appear anywhere in the query layer.
...
- Verify that fields listed in `sensitive_fields` from api-design.md front-matter are NOT included in list/bulk endpoint responses.
...
### 8. Authentication
- Verify the authentication mechanism matches `api_auth` from api-design.md front-matter.
...
- Do not suggest authentication changes that contradict the `api_auth` field from api-design.md front-matter — the authentication design is a project constraint.